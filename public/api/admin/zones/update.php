...
try {
    ...
    if (!isset($input['id']) || !isset($input['distance_from_km']) || !isset($input['distance_to_km']) || !isset($input['fee'])) {
    ...
    // Check for overlaps (excluding current zone)
    $checkStmt = $db->prepare("
        SELECT COUNT(*) as count 
        FROM delivery_zones 
        WHERE id != ? AND NOT (? >= distance_to_km OR ? <= distance_from_km)
    ");
    $checkStmt->execute([$input['id'], $input['distance_from_km'], $input['distance_to_km']]);
